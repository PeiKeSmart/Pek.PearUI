layui.define(['jquery', 'element'], function(exports) {
		...
		$("body .layui-tab[lay-filter='" + option.elem + "'] .layui-tab-title").on("contextmenu", "li",
			function(e) {
				...
				var menu = "<ul><li class='item' id='" + option.elem +
					"closeThis'>"+ layuiCloseCurrent +"</li><li class='item' id='" + option.elem +
					"closeOther'>"+ layuiCloseOther +"</li><li class='item' id='" + option.elem +
					"closeAll'>"+ layuiCloseAll +"</li></ul>";
			...
			if (isData == false) {
				...
				if (this.option.tabMax != false) {
					if ($(".layui-tab[lay-filter='" + this.option.elem + "'] .layui-tab-title li[lay-id]")
						...
						layer.msg(layuiOpenAtMost + this.option.tabMax + layuiTabs, {
							icon: 2,
							time: 1000,
							shift: 6
						});
	...
	function createTab(option) {
		...
		title += '</ul>';
		content += '</div>';
		control += '<dd id="closeThis"><a href="#">'+layuiCloseCurrent+'</a></dd>'
		control += '<dd id="closeOther"><a href="#">'+layuiCloseOther+'</a></dd>'
		control += '<dd id="closeAll"><a href="#">'+layuiCloseAll+'</a></dd>'
